...
`
        ...
        Client.cmd.on('setgroupdesc', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ text ]*\nContoh : ${data.prefix}${data.command} Jangan Rasis`)
     ...
     //           case 'menu':
            ...
            case 'groupmenu':
                    ...
                    data.reply(groupmenu(data.prefix))

                    break
                case 'downloadmenu':
                    data.reply(downloadmenu(data.prefix))
                    break
                case 'stickermenu':
                    data.reply(stickermenu(data.prefix))
                    break
                case 'markermenu':
                    data.reply(markermenu(data.prefix))
                    break
                case 'searchmenu':
                    data.reply(searchmenu(data.prefix))
                    break
                case 'educationmenu':
                    data.reply(educationmenu(data.prefix))
                    break
                case 'ownermenu':
                    data.reply(ownermenu(data.prefix))
                    break
case 'othermenu':
                    data.reply(othermenu(data.prefix))
                    break
case 'randommenu':
                    data.reply(randommenu(data.prefix))
                    break
                case 'info':
                case 'ingfo':
